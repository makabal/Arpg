...
“技能栏职责清理”（ARPG-20260920-01）已完成实现并进入 REVIEW：SkillBar / SkillSlotView 只负责当前装备技能的图标、空槽、冷却、快捷键和输入；技能解锁、允许装备与锁定展示不再属于技能栏运行时链路。技能解锁最终归属未来技能界面 / 技能树。
...
- 技能面板和技能树。
- 超过 6 个技能后的拖拽装配。
- 技能栏输入、冷却和场景引用的 Unity Play Mode 端到端验证。
- 普通敌人 AI。
- 物品、装备和背包。
- 正式存档。
- 正式资源加载流程。
...
- 2026-09-20：ARPG-20260920-01「技能栏职责清理」已完成代码、Prefab 与静态检查，进入 REVIEW；Unity 编译与 Play Mode 尚未验证。
