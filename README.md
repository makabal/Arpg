...
我已经完成训练场的基础战斗闭环，包括玩家移动、普通攻击、目标选择、角色 HUD、组合式技能框架，以及六槽技能栏的第一版运行时接入。技能栏职责清理已经完成验收；当前继续验收“剑刃风暴”，并搭建多页面角色界面和技能树页面的视觉、顶部页签与基础显隐逻辑，为后续角色页、技能点和拖拽装配做准备。
...
- [已接入，待配置与验收] 技能点、升级节点、前置条件和互斥分支运行时基础
- [已接入，待配置与验收] 数值升级、形态替换、附加效果和 Buff 技能构建链路
- [已接入，待配置与验收] Buff 持续时间、叠层、属性修改和周期伤害/治疗基础
- [进行中] 剑刃风暴长按、耗蓝、伤害范围和收尾的完整运行验收
- [未开始] 具体技能树资产、升级内容配置、UI 绑定和技能装配
...
| `PlayerSkillProgression` | 管理技能点、节点等级、前置和互斥分支 |
| `SkillBuildSnapshot` | 保存一次施法最终使用的升级数值与形态 |
| `BuffController` | 管理角色当前 Buff、持续时间和叠层 |
...
- [完成] 精简技能栏职责，移除技能栏中的解锁/锁定逻辑
- [完成] 为普通攻击配置技能栏图标
- [已接入，待验收] `Tab` 键切换 `TabPag` 显隐，打开时默认选择角色页
- [已接入，待验收] 顶部六页签的选中表现和页面切换基础逻辑
- [进行中] 技能页面视觉结构，包括技能点、重置按钮、图例和操作说明
- [未开始] 角色页面内容；当前默认选择角色页时内容区域为空
- [已接入，待 UI] 技能点、等级、分支和解锁运行时领域逻辑
- [未开始] 技能树节点交互和拖拽装配
...
- `TabPageToggleController`：监听 `Tab` 并切换 `TabPag`，打开时选择默认角色页
- `TopBarTabGroup` / `TopBarPageGroup`：顶部页签状态与页面显隐
- `Assets/Resources/UI/SkillSystem`：技能栏与世界指示器运行资源
- `Assets/Resources/UI/TopBar`：顶部栏运行资源
- `Assets/Art/UI`：UI 概念稿和制作资源

#### 下一步

- 在 Unity Play Mode 验证 `TabPag` 开关、默认角色页和顶部页签切换。
- 创建角色页面内容；当前只完成默认选择逻辑。
- 继续搭建独立技能树界面；当前技能点、重置按钮等仍是视觉初稿。
- 在独立技能界面中实现解锁、技能点、等级和分支。
- 设计玩家拥有技能与六槽技能栏之间的拖拽装配流程。
...
- `CharacterSaveData` / `SkillProgressSaveData`：角色 JSON 技能成长数据
...
- 将角色其余永久数据逐步接入版本化存档，不直接序列化运行时组件或修改 SO。
...
- [x] 清理技能栏中的解锁/锁定职责
- [ ] 创建技能树配置资产并将技能页面绑定到技能成长运行时
- [ ] 实现超过 6 个技能时的拖拽装配
- [ ] 验证 Tab 界面显隐、默认角色页和顶部页签切换
- [ ] 完成训练场碰撞与相机边界
...
- 新增技能点、升级节点、前置/互斥分支、重置和 JSON 进度 DTO。
- 新增有效技能构建快照，支持数值升级、替换技能形态和附加效果。
- 新增通用 Buff、运行时属性、叠层、持续时间和周期效果基础；Unity C# 编译已通过，尚待配置资产与 Play Mode 验收。
- 为 Input System 增加 `Tab` Action，并接入 `TabPag` 显隐控制；打开时默认选择 `Character`。
- 增加顶部六页签选中与页面切换基础逻辑；角色页面尚未制作。
- 扩充技能页面视觉结构，加入技能点、重置按钮、图例和操作说明；相关业务逻辑尚未实现。
- 新增 `Tab_Btn` Prefab 和顶部栏运行资源；本批 UI 变更仍待 Unity 编译与 Play Mode 验证。

### 2026-09-20

- 完成技能栏职责清理并通过 Unity 编译与开发者 Play Mode 人工验证；技能栏不再承担技能解锁/锁定职责。
- 为普通攻击配置技能图标，并初步加入技能树背景资源与 `SkillTreePanel` 场景基础节点；技能树运行时逻辑尚未实现或验收。
